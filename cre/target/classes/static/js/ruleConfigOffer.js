...
var initFlowObj = {
    ...
    init: function () {
        if (optMode == 'update' || optMode == 'view') {
            ...
            // $('#modelVersion').val(version); // 版本号及版本描述回显
            // $('#modelVersionDes').val(versionDesc);
        }
    ...
    // 新: (加入草稿暂存等) 规则保存调用方法 从myflow.source.js中调用的
    saveRule: function (data) {
        ...
        if (hasStart && !hasEnd) {
            failedMessager.show('一个决策树至少有一个结束节点！');
            return;
        }
        if (!hasStart && hasEnd) {
            failedMessager.show('缺失开始节点，请添加！');
            return;
        }
    ...
    bindBtnClick: function () {
        ...
        // 保存后的弹窗中点了保存按钮
        $("#editRuleName .saveAll").click(function () {
            if (!$('#editRuleName form').isValid()) {
                return;
            }
            var version = $.trim($('#modelVersion').val());
    ...
    gotoLastPageBtn: function () {
        $('#gotoLastPage, #lastPageLink').click(function () {
            ...
            if (optMode == "insert") { // pageType 0/1/2
                if (pageType === '0') {
                    hrefStr += "/create?idx=2&ruleType=" + ruleType + "&ruleFolder=" + folderId + '&childOpen=o';
                } else {
...
function getValue () {
    ...
    var moduleName = $.trim($('#ruleName').val());
...
$(function () {
    ...
    window.addEventListener('message', function (e) {
        ...
        var local = e.data.local;
        switch ( code ) {
            case 'stage':
                parent.postMessage( getValue(), local );
                break;
            case 'submit':
                parent.postMessage( getValue(), local );
                break;
