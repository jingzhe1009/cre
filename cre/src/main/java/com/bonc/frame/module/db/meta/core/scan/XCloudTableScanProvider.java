...
public class XCloudTableScanProvider implements IScan<TableInfo> {

    private IConnection conn;
    private ScanTask scanTask;

    public XCloudTableScanProvider(IConnection conn, ScanTask scanTask) throws SQLException {
        this.conn = conn;
    ...
    private List<TableInfo> scanObjectMetaInfo1(String key) throws SQLException {
        ResultSet tSet = null;
...
//					cSet = dbmd.getColumns(null, conn.getConnectionUser(), tSet.getString("TABLE_NAME"), "%");
                ...
                while (cSet.next()) {
                    ...
                    String remarks = null;
                    if (cSet.getString("REMARKS") == null || cSet.getString("REMARKS").isEmpty()
                            || cSet.getString("REMARKS").length() > 32) {
                    ...
                    structInfo.setColumnType(changeDbType(cSet.getString("DATA_TYPE")));
                    structInfo.setColumnSize(cSet.getObject("DATA_SIZE") == null ? 0
                            : cSet.getInt("DATA_SIZE"));
...
//				        structInfo.setDataType(cSet.getShort("DATA_TYPE"));
    ...
    private static String getSchema(Connection conn) throws Exception {
        ...
        if ((schema == null) || (schema.length() == 0)) {
            throw new Exception("XCLOUD数据库模式不允许为空");
        }
