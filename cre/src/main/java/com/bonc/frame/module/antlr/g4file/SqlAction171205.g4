grammar SqlAction;                            // 文法的名字为Action

//在Action.g4的基础上增加[a.b]这种适用sql的语法
//动作部分校验文件
...
temp:	'[' id=ID ('.' ID)* ']' '=' expr			#tempExpr
	  | '[' id=ID ('.' ID)*  ']' '=' funCall		#tempFunCall //expr里面已包含funCall,故此行可删除
	  | funCall							#tempFun
	  ;
...
factor: (sign=(ADD | SUB))? num=NUMBER  #minus		// 计算因子可以是一个正数或负数
	...
	| '[' id=ID ('.' ID)*  ']' 					#variable 	// 计算因子可以是自定义的参数
...
NUMBER : [0-9]+('.'([0-9]+)?)? ;        // 数字
ID : [a-zA-Z0-9_][a-zA-Z0-9_]* ;      		// 标识符，由0到多个小写字母组成
STRING : '"' ('\\"'|.)*? '"'  			// 双引号字符串
	   | '\'' ('\\"'|.)*? '\'' ; 		// 单引号字符串
EVAL:'='; //赋值
ADD : '+' ;
